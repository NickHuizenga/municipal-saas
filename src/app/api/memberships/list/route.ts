...
export async function GET(req: Request) {
  ...
  // Admin client (service role) to bypass RLS and read memberships + user emails
  const admin = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!);

  // memberships for the tenant
  ...
  // Map emails via Admin API (fetch up to first 1000 users)
  ...
  const rows = memberships.map(m => ({ user_id: m.user_id, email: emailById.get(m.user_id) ?? null, role: m.role }));
